...
function RuntimeProvider({client,children,workspaceId}:{client:ContentBusinessClient;children:ReactNode;workspaceId:string}) {
 ...
 useEffect(()=>{if(activeId)localStorage.setItem(storageKey,activeId);else localStorage.removeItem(storageKey);},[activeId,storageKey]);
...
function ContentDialogue({knowledgeEditingBlocked,conversationFooter}:ContentHomeProps){const runtime=useRuntime();const [prompt,setPrompt]=useState("");const [files,setFiles]=useState<File[]>([]);const [sending,setSending]=useState(false);const busy=sending||knowledgeEditingBlocked||["running","pending"].includes(runtime.activeConversation?.status??"");return <div className="cp-live-dialogue"><div className="cp-live-messages">{runtime.activeConversation?.messages.map(message=><article key={message.id} className={`cp-live-message cp-live-message--${message.role}`}><strong>{message.role==="user"?"我的输入":"内容制作"}</strong><div className="cp-live-text">{message.content}</div>{message.attachments?.map(file=><small key={file.id}>{file.name}</small>)}{message.outputFiles?.filter(file=>/^\/api\/frontmind\/v2\/artifacts\/[^/]+\/content(?:\?|$)/.test(file.fileUrl)).map((file,index)=><a key={file.fileUrl} href={file.fileUrl} target="_blank" rel="noreferrer" data-workbench-output-key={`${message.id}:${index}`}>{file.fileName}</a>)}{message.generalChatDispatch&&<button onClick={()=>void runtime.sendMessage(message.content).catch(()=>{})}>重试本次提交</button>}</article>)}</div>{conversationFooter}<form onSubmit={async event=>{event.preventDefault();if(!prompt.trim()||busy)return;setSending(true);try{await runtime.sendMessage(prompt,files);setPrompt("");setFiles([]);}catch{}finally{setSending(false);}}}><textarea aria-label="内容任务消息" value={prompt} onChange={event=>setPrompt(event.target.value)} disabled={busy} placeholder="补充材料或向当前内容任务提供反馈"/><input aria-label="内容任务附件" type="file" multiple disabled={busy} onChange={event=>setFiles(Array.from(event.target.files??[]))}/><div><button disabled={busy||!prompt.trim()} type="submit">发送</button>{["running","pending"].includes(runtime.activeConversation?.status??"")&&<button type="button" onClick={()=>void runtime.stop().catch(()=>{})}>停止当前执行</button>}</div></form></div>}
const useDraftGuard:ContentWorkspaceHost["useWorkspaceDraftGuard"]=({dirty})=>{useEffect(()=>{if(!dirty)return;const handler=(event:BeforeUnloadEvent)=>{event.preventDefault();event.returnValue="";};window.addEventListener("beforeunload",handler);return()=>window.removeEventListener("beforeunload",handler);},[dirty]);};
function InstalledWorkspace({client}:{client:ContentBusinessClient}){const host:ContentWorkspaceHost={connections:{publishedKnowledge:false},ConversationPurposeProvider:({children})=><>{children}</>,useConversation:useRuntime,useSendMessage:()=>({sendMessage:useRuntime().sendMessage}),retrieveTask:(id,options)=>client.task(id,options?.signal),captureWorkspaceRestOperation:(signal=new AbortController().signal)=>({signal,assertActive(){signal.throwIfAborted();},fetch:(url,init)=>fetch(url,{...init,signal})}),useWorkspaceDraftGuard:useDraftGuard,requestWorkspaceNavigation:operation=>operation(),Home:ContentDialogue,FilePreview:({file,className})=><a className={className} href={file.blobUrl} target="_blank" rel="noreferrer">{file.name}</a>,WorkbenchTaskToolbar:({tasks,currentId,onNew,onSelect,onDelete,disabled})=><div className="cp-live-history"><button onClick={onNew} disabled={disabled}>新建任务</button>{tasks.map(task=><div key={task.id}><button aria-current={currentId===task.id} onClick={()=>onSelect(task.id)} disabled={disabled}>{task.title}</button><button aria-label={`删除 ${task.title}`} onClick={()=>onDelete(task.id)} disabled={disabled}>删除</button></div>)}</div>,AgentWorkbenchShell:({main,auxiliary,topbarActions})=><section>{topbarActions}{main}{auxiliary}</section>,BusinessWorkspaceInspector:({summary})=><aside>{summary.status}{summary.outputs.map(item=><button key={item.id} onClick={item.onOpen}>{item.title}</button>)}</aside>};return <ContentWorkspaceHostProvider value={host}><ContentProductionWorkspace/></ContentWorkspaceHostProvider>}
