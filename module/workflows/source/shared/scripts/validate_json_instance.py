...
disk inside the entry-schema directory or the workflow's canonical ``shared``
schema directory; network, absolute and other escaping references are rejected.
"""
...
class JsonSchemaValidator:
    ...
    def __init__(self, schema_path: Path, schema: Any = _SCHEMA_NOT_SUPPLIED):
        ...
        self.canonical_schema_root = Path(__file__).resolve().parents[1]
    ...
    def _resolve_ref(self, ref: str, current_document: Path) -> tuple[Any, Path, str]:
        ...
        if file_part:
            ...
            if (
                not target.is_relative_to(self.reference_root)
                and not target.is_relative_to(self.canonical_schema_root)
            ):
                raise ReferenceResolutionError(f"$ref escapes schema and canonical shared directories: {ref!r}")
        else:
