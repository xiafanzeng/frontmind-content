...
- P01/P02 相同条件沿用已确认比较依据，条件变化说明调整；本次供应商、模型与推理档位由任务执行配置冻结，真实验收报告记录实际值；
...
旧版 glm-5.3 集中表达修订的主运行、独立复测、失败与针对性修正、跨行业和 P02 观察见历史报告，仅供对照。当前任务配置的真实验收应单独报告；程序通过不表示实际文字达标。没有实测报告时应视为尚未验证真实生成质量，不以示意段落或手工改写替代，也不增加生产调用、语义评分或用户审批。
