...
|---|---|---|---|---|
| 任务输入 | 已包含：内容工作台、问题与材料输入、回答/参考文件引用；业务输入和文件归属校验。 | `module/client/ContentProductionWorkspace.tsx`；`module/server/task-handlers.ts`；`module/contracts/` | 见下方本地检查；不据此推定真实业务通过。 | 真实材料上传、输入保存和创建任务待验收。 |
| Job 与持久化 | 已包含：内容业务会话、任务记录、冻结输入快照、状态和持久化接口。 | `module/server/conversations.ts`；`module/server/task-persistence.ts`；`module/server/persistence.ts`；`module/server/state.ts` | 见下方本地检查；不据此推定真实业务通过。 | 真实任务创建、保存、刷新恢复和继续执行未验收。 |
...
| 范围 | 已有证据与待完成项 |
|---|---|
| 目标版本页面 | 旧业务版本在会话接口修复后已通过首页、内容页、深链与刷新，未出现会话接口 500；新弹窗修复版本仍待目标镜像的页面复验。 |
| 普通保存与刷新 | 新版本弹窗的本地预览检查通过；子域名输入保存、上传、任务创建和刷新恢复尚无完整真实业务证据。 |
...
工作台可打开和弹窗预览通过，不证明任务已运行；确认、恢复、成果文件授权必须以真实任务逐项验收。
